...
# Add scroll position management and page top anchor
st.markdown('<div id="top-anchor"></div>', unsafe_allow_html=True)

# Initialize mode state management
if 'current_mode' not in st.session_state:
    st.session_state.current_mode = 'chat'
if 'saved_messages' not in st.session_state:
    st.session_state.saved_messages = []

# Add CSS for optimized layout and scroll management
st.markdown("""
<style>
...
/* Admin content container with internal scrolling */
...
header_col1, header_col2, header_col3 = st.columns([4, 3, 3])

with header_col1:
    # Logo and Note side by side
    logo_col, note_col = st.columns([1, 2])
    with logo_col:
        import os
        gif_path = os.path.abspath("gsk_logo_animated.gif")
        if os.path.exists(gif_path):
            st.image(gif_path, width=80, caption="")
        else:
            st.markdown("**GSC**")
...
with header_col2:
    # User Email (centered)
    user_email = st.session_state.get('email_id', 'User')
    st.markdown(f"""
...
with header_col3:
    ...
    with control_col1:
        if st.button("🔄 New", key="header_new_session", help="New Session"):
            try:
    ...
    with control_col2:
        if st.button("🗑️ Clear", key="header_clear_session", help="Clear Session"):
...
# Delete button
        ...
        # Main app logic - show admin panel or regular chat with mode management
        if is_admin and st.sidebar.button("Admin Panel", use_container_width=True):
            # Save current chat messages and switch to admin mode
            if st.session_state.current_mode == 'chat':
                st.session_state.saved_messages = st.session_state.messages.copy()
            st.session_state.current_mode = 'admin'
            st.session_state.show_admin = True
            # Scroll to top when entering admin mode
            st.markdown('<script>scrollToTop();</script>', unsafe_allow_html=True)
            st.rerun()

        if st.session_state.get('show_admin') and is_admin:
            # Show admin panel with scroll management
            if st.sidebar.button("💬 Back to Chat", use_container_width=True):
                # Restore chat messages and switch to chat mode
                st.session_state.current_mode = 'chat'
                if st.session_state.saved_messages:
                    st.session_state.messages = st.session_state.saved_messages.copy()
                st.session_state.show_admin = False
                st.rerun()
            
            # Admin content with internal scrolling container
            st.markdown('<div class="admin-content-container">', unsafe_allow_html=True)
            render_admin_panel()
            ...
            try:
                ...
                if status:
                    # Add CSS for uniform FAQ button widths using containers
                    st.sidebar.markdown("""
                    <style>
                    ...
                    # Create FAQ buttons using containers for uniform width
                    for i, (q, cnt) in enumerate(st.session_state["faqs"].items()):
                        # Create a container to constrain button width
        ...
        # Only show chat content when in chat mode
        if st.session_state.current_mode == 'chat':
            # Chat content area
            pass  # Chat content will be rendered below
 
            # Display chat messages only in chat mode
            if st.session_state.current_mode == 'chat':
